...
### Version
Each SignatureRecord also carries a Version: which SignatureCanonicalSerializer schema (field set and format) computed its SignatureHmac. It is set once at signing time to `SignatureCanonicalSerializer.CurrentVersion` and never changes afterward. This exists so that if the canonical schema ever changes — a field is added, removed, or reformatted — verification can still reconstruct the exact format a given signature was made under, instead of hashing today's field set against a signature made under an older one. Each schema version's serialization logic is frozen in code forever once real signatures exist under it; a schema change adds a new version rather than editing an existing one. Version is bookkeeping only — it is never part of the hashed canonical input itself, so it has no bearing on HMAC verification, and it is **not** a resign counter: many signatures legitimately share the same Version (they were all made under the same schema). How many times a training slot has been re-signed is instead derived from ordering its SignatureRecords by SignedAt.
